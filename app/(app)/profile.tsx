...
/**
 * The highlights row: the achievements the owner chose to display, then a
 * dashed "More" that opens /achievements — always present, so the screen is
 * discoverable even before the first badge is unlocked.
 */
function Badges({ dark, achievements, achievementCount, onMore }: { dark: boolean; achievements: DisplayedAchievements; achievementCount: number; onMore: () => void }) {
  const palette = dark ? darkColors : lightColors;
  const hidden = Math.max(0, achievementCount - achievements.length);
  const data: { key: string; icon: keyof typeof Ionicons.glyphMap; label: string; more?: boolean }[] = [
    ...achievements.map((a) => ({ key: a.id, icon: badgeIcons[a.icon] ?? "trophy-outline", label: a.name })),
    { key: "more", icon: "add-outline", label: hidden > 0 ? `+${hidden} more` : "More", more: true },
  ];
  return <View style={s.badges}><FlatList horizontal showsHorizontalScrollIndicator={false} data={data} keyExtractor={(item) => item.key} contentContainerStyle={s.badgesContent} renderItem={({ item }) => <Pressable onPress={item.more ? onMore : undefined} disabled={!item.more} accessibilityRole={item.more ? "button" : undefined} accessibilityLabel={item.more ? "More achievements" : item.label} style={s.badgeItem}><View style={[s.badgeCircle, { backgroundColor: palette.surface, borderColor: item.more ? palette.muted : (dark ? darkColors.border : color.controlLine) }, item.more && s.moreBadge]}><Ionicons name={item.icon} size={24} color={item.more ? palette.muted : palette.green} /></View><Text style={[s.badgeLabel, { color: palette.muted }]} numberOfLines={1} ellipsizeMode="tail">{item.label}</Text></Pressable>} /></View>;
}
